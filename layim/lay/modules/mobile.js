...
layui.define(["laytpl", "upload", "layer-mobile", "zepto"], function (i) {
	var e = "2.5.0",
		...
		d = {},
		r = function () {
			this.v = e, u(a("body"), "*[layim-event]", function (i) {
				var e = a(this),
		...
		u = function (i, e, t) {
			var n,
				...
				s = function (i) {
					var e = a(this);
					e.data("lock") || (n || t.call(this, i), n = !1, e.data("lock", "true"), setTimeout(function () {
						e.removeAttr("data-lock")
					}, e.data("locktime") || 0))
				};
			return l && (t = e), i = "string" == typeof i ? a(i) : i, m ? void (l ? i.on("touchmove", function () {
				n = !0
	...
	n.popBottom = function (i) {
		n.close(n.popBottom.index), n.popBottom.index = n.open(a.extend({
			type: 1,
			content: i.content || "",
			shade: !1,
		...
		return "function" == typeof e && (d[i] ? d[i].push(e) : d[i] = [e]), this
	}, r.prototype.chat = function (i) {
		if (window.JSON && window.JSON.parse) return N(i, -1), this
	...
	var g,
		b,
		x = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">', '<div class="layim-chat-user"><img src="{{ d.avatar }}" alt="{{ d.uid || d.id }}"><cite>', '{{ d.username||"佚名" }}', "</cite></div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp;") }}</div>', "</li>"].join(""),
		w = {
			message: {},
			chat: []
		},
		C = function (i) {
			var e = i.init || {};
			return mine = e.mine || {}, local = layui.data("layim-mobile")[mine.id] || {}, obj = {
				base: i,
				local: local,
				mine: mine,
				...
				s.sortHistory = A(s.history, "historyTime"), w = a.extend(w, s), k(t(p(f)).render(s)), layui.each(d.ready, function (i, e) {
					e && e(s)
				})
			}, w = a.extend(w, obj), i.brief ? layui.each(d.ready, function (i, e) {
				e && e(obj)
			}) : void create(e)
		},
		k = function (i) {
			return n.open({
				...
				content: i,
				success: function (i) {
					g = a(i), L(g.find(".layui-layim")), w.base.tabIndex && R.tab(a(".layui-layim-tab>li").eq(w.base.tabIndex))
				}
			})
		},
		S = function (i, e) {
			...
			return n.open({
				...
				success: function (e) {
					var t = a(e);
					t.prev().find(".layim-panel").addClass("layui-m-anim-lout"), i.success && i.success(e), i.isChat || L(t.find(".layim-content"))
				},
				end: i.end
			})
		},
		N = function (i, e, t) {
			return i = i || {}, i.id ? (n.close(N.index), N.index = S({
				tpl: h,
				data: i,
				title: i.name,
				isChat: !0,
				success: function (e) {
					b = a(e), J(), M(), delete w.message[i.type + i.id], H("Msg");
					var t = T(),
						n = t.elem.find(".layim-chat-main");
					layui.each(d.chatChange, function (i, e) {
						e && e(t)
					}), L(n), t.textarea.on("focus", function () {
						setTimeout(function () {
		...
		j = function (i) {
			...
			if (g) {
				...
				i.historyTime = (new Date).getTime(), i.sign = i.content, n[i.type + i.id] = i, e.history = n, layui.data("layim-mobile", {
					key: w.mine.id,
					value: e
				});
				var o = l.find(".layim-" + i.type + i.id),
					...
					d = function () {
						o = l.find(".layim-" + i.type + i.id), o.find("p").html(i.content), c > 0 && o.find(".layim-msg-status").html(c).addClass(s)
					};
		...
		q = function () {
			...
			if (i.content = c.val(), "" !== i.content) {
				...
				o - (q.time || 0) > 6e4 && (a.append('<li class="layim-chat-system"><span>' + layui.data.date() + "</span></li>"), q.time = o), a.append(t(x).render(i));
				var r = {
						mine: i,
						to: l
					},
				...
				F(u), layui.each(d.sendMessage, function (i, e) {
					e && e(r)
				}), l.content = i.content, j(l), Y(), c.val(""), c.next().addClass("layui-disabled")
			}
		},
		I = function () {
			var i = document.createElement("audio");
			i.src = layui.cache.dir + "css/modules/layim/voice/" + w.base.voice, i.play()
		},
		D = {},
		_ = function (i) {
			i = i || {};
			...
			i.timestamp = i.timestamp || (new Date).getTime(), i.system || F(i), console.log(i), D = JSON.parse(JSON.stringify(i)), w.base.voice && I(), (!b && i.content || !s) && (w.message[i.type + i.id] ? w.message[i.type + i.id].push(i) : w.message[i.type + i.id] = [i]);
			var e = {};
			if ("friend" === i.type) {
				var o;
				layui.each(w.friend, function (e, a) {
					if (layui.each(a.list, function (e, a) {
						if (a.id == i.id) return i.type = "friend", i.name = a.username, o = !0
					}), o) return !0
				}), o || (i.temporary = !0)
			} else "group" === i.type ? layui.each(w.group, function (a, t) {
				if (t.id == i.id) return i.type = "group", i.name = i.groupname = t.groupname, e.avatar = t.avatar, !0
			}) : i.name = i.name || i.username || i.groupname;
			var c = a.extend({}, i, {avatar: e.avatar || i.avatar});
			if ("group" === i.type && delete c.username, j(c), b && s) {
				var d = b.find(".layim-chat"),
					r = d.find(".layim-chat-main ul");
				i.system ? r.append('<li class="layim-chat-system"><span>' + i.content + "</span></li>") : "" !== i.content.replace(/\s/g, "") && (i.timestamp - (q.time || 0) > 6e4 && (r.append('<li class="layim-chat-system"><span>' + layui.data.date(i.timestamp) + "</span></li>"), q.time = i.timestamp), r.append(t(x).render(i))), Y()
			}
		},
		F = function (i) {
			var e = layui.data("layim-mobile")[w.mine.id] || {},
				a = e.chatlog || {};
			a[i.type + i.id] ? (a[i.type + i.id].push(i), a[i.type + i.id].length > c && a[i.type + i.id].shift()) : a[i.type + i.id] = [i], e.chatlog = a, layui.data("layim-mobile", {
				key: w.mine.id,
				value: e
			})
		},
		M = function () {
			...
			layui.each(a[e.data.type + e.data.id], function (i, e) {
				(new Date).getTime() > e.timestamp && e.timestamp - (q.time || 0) > 6e4 && (n.append('<li class="layim-chat-system"><span>' + layui.data.date(e.timestamp) + "</span></li>"), q.time = e.timestamp), n.append(t(x).render(e))
			}), Y()
		},
		$ = function (i) {
			var e,
				a = {},
				l = g.find(".layim-list-" + i.type);
			if (w[i.type]) if ("friend" === i.type) layui.each(w.friend, function (t, l) {
				if (i.groupid == l.id) return layui.each(w.friend[t].list, function (a, t) {
					if (t.id == i.id) return e = !0
				}), e ? n.msg("好友 [" + (i.username || "") + "] 已经存在列表中", {anim: 6}) : (w.friend[t].list = w.friend[t].list || [], a[w.friend[t].list.length] = i, i.groupIndex = t, w.friend[t].list.push(i), !0)
			}); else if ("group" === i.type) {
				if (layui.each(w.group, function (a, t) {
					if (t.id == i.id) return e = !0
				}), e) return n.msg("您已是 [" + (i.groupname || "") + "] 的群成员", {anim: 6});
				a[w.group.length] = i, w.group.push(i)
			}
			if (!e) {
				var s = t(y({
					type: i.type,
				...
				if ("friend" === i.type) {
					...
					o.find(".layui-layim-list").append(s), o.find(".layim-count").html(w.friend[i.groupIndex].list.length), o.find(".layim-null")[0] && o.find(".layim-null").remove()
			...
			w[i.type] && ("friend" === i.type ? layui.each(w.friend, function (a, t) {
				layui.each(t.list, function (t, n) {
					if (i.id == n.id) {
						...
						return l.find(".layui-layim-list").children("li").eq(t).remove(), w.friend[a].list.splice(t, 1), l.find(".layim-count").html(w.friend[a].list.length), 0 === w.friend[a].list.length && l.find(".layui-layim-list").html('<li class="layim-null">该分组下已无好友了</li>'), !0
					}
				})
			}) : "group" === i.type && layui.each(w.group, function (a, t) {
				if (i.id == t.id) return e.children("li").eq(a).remove(), w.group.splice(a, 1), 0 === w.group.length && e.html('<li class="layim-null">暂无群组</li>'), !0
			}))
		},
		Y = function () {
			var i = T(),
			...
			if (t.length >= c) {
				var n = t.eq(0);
				n.prev().remove(), a.prev().hasClass("layim-chat-system") || a.before('<div class="layim-chat-system"><span layim-event="chatLog">查看更多记录</span></div>'), n.remove()
			}
		...
		J = function () {
			...
			e.off("keyup").on("keyup", function (i) {
				var t = i.keyCode;
				13 === t && (i.preventDefault(), q()), a["" === e.val() ? "addClass" : "removeClass"]("layui-disabled")
			})
		},
		B = function () {
		...
		P = function (i, e, a) {
			var t,
				n = i.value;
			a || i.focus(), document.selection ? (t = document.selection.createRange(), document.selection.empty(), t.text = e) : (t = [n.substring(0, i.selectionStart), e, n.substr(i.selectionEnd)], a || i.focus(), i.value = t.join(""))
		},
		R = {
			chat: function (i) {
				var e = layui.data("layim-mobile")[w.mine.id] || {},
					...
					o = {};
				fetch("/api/chatApi/readRecord", {method: "POST", body: JSON.stringify({'friendId':friendId})});
				"friend" === t ? o = w[t][n].list[l] : "group" === t ? o = w[t][l] : "history" === t && (o = (e.history || {})[n] || {}), o.name = o.name || o.username || o.groupname, "history" !== t && (o.type = t), N(o, !0), a(".layim-" + o.type + o.id).find(".layim-msg-status").removeClass(s)
			},
			spread: function (i) {
				var e = i.attr("lay-type"),
					a = "true" === e ? "false" : "true",
					t = layui.data("layim-mobile")[w.mine.id] || {};
				i.next()["true" === e ? "removeClass" : "addClass"](s), t["spread" + i.parent().index()] = a, layui.data("layim-mobile", {
					key: w.mine.id,
					value: t
				}), i.attr("lay-type", a), i.find(".layui-icon").html("true" === a ? "&#xe61a;" : "&#xe602;")
			},
			tab: function (i) {
				var e = i.index(),
					a = ".layim-tab-content";
				i.addClass(o).siblings().removeClass(o), g.find(a).eq(e).addClass(s).siblings(a).removeClass(s)
			},
			back: function (i) {
				...
				setTimeout(function () {
					n.close(a)
				}, 300), i.parents(t).eq(0).removeClass("layui-m-anim-left").addClass("layui-m-anim-rout"), e.prev().find(t).eq(0).removeClass("layui-m-anim-lout").addClass("layui-m-anim-right"), layui.each(d.back, function (i, e) {
					setTimeout(function () {
						e && e()
					}, 200)
				})
			...
			face: function (i, e) {
				...
				layui.each(B, function (i, e) {
					...
					success: function (i) {
						...
						u(e, function () {
							return P(s[0], "face" + this.title + " ", !0), s.next()["" === s.val() ? "addClass" : "removeClass"]("layui-disabled"), !1
						})
					}
				});
				var o = a(document);
				m ? u(o, function () {
					R.faceHide()
				}) : o.off("click", R.faceHide).on("click", R.faceHide), E(e)
			},
			faceHide: function () {
				n.close(n.popBottom.index), a(document).off("touchend", R.faceHide).off("click", R.faceHide)
			},
			image: function (i) {
				...
				layui.upload.render({
					...
					accept: e,
						...
						0 == i.code ? (i.data = i.data || {}, "images" === e ? P(t.textarea[0], "img[" + (i.data.src || "") + "]") : "file" === e && P(t.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]"), q()) : n.msg(i.msg || "上传失败")
					}
				})
			},
			extend: function (i) {
			...
			playAudio: function (i) {
				var e = i.data("audio"),
					...
					t = function () {
						a.pause(), i.removeAttr("status"), i.find("i").html("&#xe652;")
					};
				return i.data("error") ? n.msg("播放音频源异常") : a.play ? void (i.attr("status") ? t() : (e || (a.src = i.data("src")), a.play(), i.attr("status", "pause"), i.data("audio", a), i.find("i").html("&#xe651;"), a.onended = function () {
					t()
				}, a.onerror = function () {
					n.msg("播放音频源异常"), i.data("error", !0), t()
				})) : n.msg("您的浏览器不支持audio")
			},
			playVideo: function (i) {
				var e = i.data("src"),
					a = document.createElement("video");
				return a.play ? (n.close(R.playVideo.index), void (R.playVideo.index = n.open({
					type: 1,
					anim: !1,
			...
			about: function () {
				n.open({
					content: '<p style="padding-bottom: 5px;">版本：layim mobile v' + e + "</p>",
...
}).addcss("modules/layim/mobile/layim.css?v=2.50", "skinlayim-mobilecss");
