    ...
    function (i) {
        ...
        c.prototype.config = function (i) {
            ...
            c.prototype.chat = function (i) {
                if (window.JSON && window.JSON.parse) return z(i),
                    this
            },
            ...
            c.prototype.setFriendStatus = function (id, status) {
                let t = e(".layim-friend" + id);
                t["online" === status ? "removeClass" : "addClass"]("layim-list-gray");
        ...
        var u = function (i) {
                ...
                return i = i || {},
                    ...
                    [
                        ...
                        '<li layim-event="chat" data-type="' + i.type + '" data-index="{{ ' + (i.index || "i") + ' }}" class="layim-' + ("history" === i.type ? "{{i}}" : i.type + "{{data.id}}") + ' {{ data.status === "offline" ? "layim-list-gray" : "" }}" data-status="{{ data.status }}"><img src="{{ data.avatar }}"><span>{{ data.username||data.groupname||data.name||"佚名" }}</span><p>{{ data.remark||data.signature||"" }}</p><span class="layim-msg-status">new</span></li>', "{{# }); if(length === 0){ }}",
                        '<li class="layim-null">' + (a[i.type] || "暂无数据") + "</li>",
                        "{{# } }}"
                    ].join("")
            },
        ...
        layui.data.date = function (i) {
            ...
            layui.data.content = function (i) {
                ...
                return i = (i || "").replace(/&(?!#?[a-zA-Z0-9]+;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/'/g, "&#39;").replace(/"/g, "&quot;").replace(/@(\S+)(\s+?|$)/g, '@<a href="javascript:;">$1</a>$2').replace(/face\[([^\s\[\]]+?)\]/g,
                    function (i) {
        ...
        var x, b, w, k, C, S = function (i, a, n) {
                return i = i || {},
                    e.ajax({
                        ...
                        cache: !1,
                        success: function (i) {
                            100000 === i.code ? a && a(i.data || {}) : t.msg(i.msg || (n || "Error") + ": LAYIM_NOT_GET_DATA", {
                                time: 5e3
                            })
                        },
                        error: function (i, a) {
            ...
            L = function (i) {
                return t.open({
                    ...
                    success: function (i) {
                        ...
                        j.base.top && i.css("top", j.base.right),
            ...
            z = function (i) {
                ...
                if (a[0]) {
                    ...
                    o[0] || (s.append(n(v).render(l)), c.append(n(f).render(l)), I(i), E()),
                        O(s.find(".layim-chatlist-" + i.type + i.id)),
                    o[0] || U(),
                        $(i),
                        Q(),
            ...
            K = function (i) {
                ...
                if (i.timestamp = i.timestamp || (new Date).getTime(), i.fromid === j.mine.id && (i.mine = !0), i.system || B(i), G = JSON.parse(JSON.stringify(i)), j.base.voice && F(), !w && i.content || l === -1) {
                    if (j.message[i.type + i.id]) j.message[i.type + i.id].push(i);
            ...
            ti = {
                ...
                tab: function (i) {
                    ...
                    "number" === typeof i ? (a = i, i = t.eq(a)) : a = i.index(),
                        a > 2 ? t.removeClass(o) : (ti.tab.index = a, i.addClass(o).siblings().removeClass(o)),
                        x.find(e).eq(a).addClass(s).siblings(e).removeClass(s)
                },
                spread: function (i) {
                    let friendGroupUpDownStatusOld = i.attr("lay-type"),
                        friendGroupUpDownStatusNew = "true" === friendGroupUpDownStatusOld ? "false" : "true",
                        cacheFriendGroupUpDownStatus = layui.data("layim")[j.mine.id] || {};
                    i.next()["true" === friendGroupUpDownStatusOld ? "removeClass" : "addClass"](s),
                        cacheFriendGroupUpDownStatus["spread" + i.parent().index()] = friendGroupUpDownStatusNew,
                        layui.data("layim", {
                            key: j.mine.id,
                            value: cacheFriendGroupUpDownStatus
                        }),
                        i.attr("lay-type", friendGroupUpDownStatusNew),
                        i.find(".layui-icon").html("true" === friendGroupUpDownStatusNew ? "&#xe61a;" : "&#xe602;")
                },
                search: function (i) {
                    var a = x.find(".layui-layim-search"),
                        ...
                        n = function (i) {
                            ...
                            if ("" === a) ti.tab(0 | ti.tab.index);
                                ...
                                if (n.length > 0) for (var u = 0; u < n.length; u++) o += '<li layim-event="chat" data-type="' + n[u].type + '" data-index="' + n[u].index + '" data-list="' + n[u].list + '"><img src="' + n[u].avatar + '"><span>' + (n[u].username || n[u].groupname || "佚名") + "</span><p>" + (n[u].remark || n[u].sign || "") + "</p></li>";
                    ...
                    layui.upload.render({
                        ...
                        accept: a,
                            ...
                            0 === i.code ? (i.data = i.data || {},
                                "images" === a ? ai(n.textarea[0], "img[" + (i.data.src || "") + "]") : "file" === a && ai(n.textarea[0], "file(" + (i.data.src || "") + ")[" + (i.data.name || "下载文件") + "]"), D()) : t.msg(i.msg || "上传失败")
                        }
                    })
                },
                media: function (i) {
        ...
        i("layim", new c)
    }).addcss("modules/layim/layim.css?v=3.9.0", "skinlayimcss");